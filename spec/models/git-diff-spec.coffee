...
describe "GitDiff", ->
  beforeEach ->
    atom.config.set 'git-plus-leap.includeStagedDiff', true
  ...
  describe "when git-plus-leap.includeStagedDiff config is true", ->
    it "calls git.cmd and specifies 'HEAD'", ->
      expect('HEAD' in git.cmd.mostRecentCall.args[0]).toBe true

describe "GitDiff when git-plus-leap.wordDiff config is true", ->
  beforeEach ->
    atom.config.set 'git-plus-leap.wordDiff', true
    atom.config.set 'git-plus-leap.includeStagedDiff', true
    spyOn(atom.workspace, 'getActiveTextEditor').andReturn textEditor
    spyOn(atom.workspace, 'open').andReturn Promise.resolve textEditor
    spyOn(git, 'cmd').andReturn Promise.resolve('diffs')
...
# describe "when git-plus-leap.openInPane config is true", ->
#   beforeEach ->
#     atom.config.set 'git-plus-leap.openInPane', true
...
#       GitDiff repo, file: '.'
#
#   describe "when git-plus-leap.splitPane config is not set", ->
